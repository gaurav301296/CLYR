...
function ScoreGauge({ score, health, t, handleDownloadPDF, downloading, reportData }) {
  ...
  return (
    <div className="card score-card" role="region" aria-label={t('scoreSummary')}>
      ...
      <ul className="client-meta-list" aria-label="Client information">
        <li className="client-meta-item">
          <span className="client-meta-label">{t('customerName')}</span>
          <span className="client-meta-value">{reportData.customer_name}</span>
        </li>
        <li className="client-meta-item">
          <span className="client-meta-label">{t('bureauHealth')}</span>
...
function SimulatorCard({ t, reportData, simulatorResolutions, setSimulatorResolutions, utilizationSlider, setUtilizationSlider, simulatedScore, simulatedScoreDelta }) {
  return (
    <div className="simulator-card" role="region" aria-labelledby="sim-title">
      <h4 id="sim-title" style={{ fontSize: '16px', color: 'var(--text-highlight)', fontWeight: '600', marginBottom: '4px' }}>{t('simTitle')}</h4>
      <p style={{ fontSize: '12.5px', color: 'var(--text-muted)', marginBottom: '20px' }}>{t('simSubtitle')}</p>
      <div className="simulator-grid">
        <div className="simulator-gauge" role="img" aria-label={`Estimated future score: ${simulatedScore}. ${simulatedScoreDelta > 0 ? `+${simulatedScoreDelta} point improvement` : simulatedScoreDelta < 0 ? `${simulatedScoreDelta} point decrease` : 'No change'}.`}>
          <div className={`sim-score-wrapper ${simulatedScoreDelta > 0 ? 'boosted' : ''}`}>
            <span className="sim-score-value">{simulatedScore}</span>
            <span className="sim-score-label">{t('estScore')}</span>
          </div>
          {simulatedScoreDelta > 0 ? (
            <span className="score-delta-badge">+{simulatedScoreDelta} {t('boost')}</span>
          ) : simulatedScoreDelta < 0 ? (
            <span className="score-delta-badge neutral" style={{ backgroundColor: 'var(--color-red-bg)', color: 'var(--color-red)', borderColor: 'rgba(239, 68, 68, 0.2)' }}>
              {simulatedScoreDelta} {t('drop')}
            </span>
          ) : (
            <span className="score-delta-badge neutral">{t('noChange')}</span>
          )}
        </div>
        <div className="simulator-details">
          <div className="sim-input-section">
            <div className="sim-input-label-row">
              <label htmlFor="utilization-slider" className="sim-input-label">{t('simUtilization')}</label>
            ...
            />
            <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: '4px 0 0' }}>{t('simUtilizationHint')}</p>
          </div>
          <fieldset className="sim-input-section" style={{ border: 'none', padding: 0, margin: 0 }}>
            <legend className="sim-input-label">{t('simResolveOverdue')}</legend>
              ...
              {reportData.issues.map((issue, idx) => {
                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); const next = new Set(simulatorResolutions); if (next.has(idx)) next.delete(idx); else next.add(idx); setSimulatorResolutions(next); } }}
                  >
                    <div className="sim-checkbox-check" aria-hidden="true">
                      {isChecked && <div style={{ width: '6px', height: '6px', backgroundColor: 'var(--primary)', borderRadius: '50%' }}></div>}
                    </div>
                    <span className="sim-checkbox-text">{t('resolveOverdueFor')} <strong>{issue.account}</strong></span>
          ...
          <div className="sim-notes" role="note">{t('simDisclaimer')}</div>
        </div>
      </div>
    </div>
  );
...
export default function DashboardPage({
  ...
  return (
    <div className="results-grid" role="main" aria-label={t('analysisDashboard')}>
      <div className="panel-sticky">
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <button className="btn btn-primary" onClick={handleDownloadPDF} disabled={downloading} aria-busy={downloading}>
            {downloading ? (
              <><RefreshCw className="spinner" size={16} style={{ margin: 0, width: 16, height: 16, borderThickness: 2 }} aria-hidden="true" /> {t('compilingBtn')}</>
            ) : (
              <><Download size={16} aria-hidden="true" /> {t('downloadBtn')}</>
            )}
          </button>
          <p style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center' }}>{t('downloadDesc')}</p>
        </div>
      </div>
        ...
        <section aria-labelledby="issues-heading">
          <div className="panel-header">
            <h3 id="issues-heading" className="panel-title">
              ...
              {t('flaggedAccounts')} ({reportData.issues.length})
            </h3>
          </div>
          <div className="issue-list" style={{ marginTop: '16px' }} role="list">
            {reportData.issues.map((issue, idx) => (
              <div key={idx} className="issue-item" role="listitem">
                <div className={`issue-bar ${['critical', 'red'].includes(issue.type.toLowerCase()) ? 'red' : 'yellow'}`} aria-hidden="true"></div>
                <div className="issue-body">
                  ...
                  <div className="issue-detail-row">
                    <div className="issue-label">{t('problemSpotted')}</div>
                  ...
                  <div className="issue-detail-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div>
                      <div className="issue-label">{t('targetImpact')}</div>
                      <div className="issue-desc" style={{ fontSize: '13.5px', fontWeight: '500' }}>{issue.impact}</div>
                    </div>
                    <div>
                      <div className="issue-label">{t('actionRequired')}</div>
        ...
        <section aria-labelledby="checklist-heading">
          <div className="panel-header">
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-muted)' }} aria-live="polite">
              {checkedTasks.size} {t('of')} {reportData.action_steps.length} {t('completed')}
            </span>
          </div>
          <div className="checklist-container" style={{ marginTop: '16px' }} role="list" aria-label="Dispute action checklist">
            {reportData.action_steps.map((step, idx) => {
